...
public class SurveyPrompt : ComponentBase
{
    protected override void BuildRenderTree(RenderTreeBuilder builder) {}
        ...
        public async Task IncrementalCompilation_RazorFiles_CssScopeRemoved()
        {
            ...
            Assert.True(result.GeneratedSources.Any(s => s.SourceText.ToString().Contains("<h1 test-css-scope>Hello world</h1>")));
            ...
            // CSS isolation is disabled but the implementation output is still cached
            // with the CSS scope. This is a known issue with the incremental pipeline
            // where the implementation output is not invalidated by config option changes.
            Assert.True(result.GeneratedSources.Any(s => s.SourceText.ToString().Contains("<h1 test-css-scope>Hello world</h1>")));
        }
...
");
        ...
        public async Task RazorClassLibrary_Change_Updates_DependentProject_WhenReferencedAsCompilation()
        {
            ...
            Assert.Equal(2, rclRun.GeneratedSources.Length); // LibComponent decl + impl
            ...
            Assert.Empty(rclRun.Diagnostics);
            Assert.Equal(2, rclRun.GeneratedSources.Length); // RenamedComponent decl + impl
            ...
            mainRun = RunGenerator(mainCompilation!, ref mainDriver,
                // Pages/Index_razor.g.cs.render.g.cs: error CS0234: The type or namespace name 'LibComponent' does not exist in the namespace 'MyApp'
                Diagnostic(ErrorCode.ERR_DottedTypeNameNotFoundInNS, "LibComponent").WithArguments("LibComponent", "MyApp").WithLocation(21, 51));
            Assert.Empty(mainRun.Diagnostics);
